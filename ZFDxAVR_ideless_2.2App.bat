:: ####################################################################################
:: Flyandance Advanced 8-bit AVR compile and Upload Batch Program Universal 2.2
...
FOR /f "delims=" %%f IN ('dir /b /s "*.c" 2^>nul') DO ( set "Obj_files=!Obj_files!"./Output/%%~nf.o" "
...
echo     ^<^< C++ Project:%Project_Name%   ^|^|   MCU:%Picked_MCU% ^>^>
echo ---------------------------------------------------------------
FOR /f "delims=" %%f IN ('dir /b /s "*.cpp" 2^>nul') DO ( set "Obj_files=!Obj_files!"./Output/%%~nf.o" "
	set "source_files=!source_files!%%~nxf "
